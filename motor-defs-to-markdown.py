...
DESCRIPTION = 'Parse ev3dev *_motor_defs.c files and convert to markdown ' \
...
def parse_file(kdir, file_name, name_constants):
    ...
    while i < len(lines):
        match = re.match('(?:\w+\s)+(\w+)_motor_defs\[\w*\]\s=\s\{', lines[i])
        i += 1
        source_line = i
        if match:
...
def main():
    ...
    for file_name in source_file_names:
        motor_list += parse_file(args.kernel, file_name, name_constants)

    # sort by vendor_name, then by vendor_part_number
    motor_list.sort(key=lambda motor: get_motor_sort_key(motor))

    # generate json data file
    with open(args.website + '/_data/motors.json', 'w') as out_file:
        print(json.dumps(motor_list, sort_keys=True, indent=4), file=out_file)

    # generate markdown files, one for each motor type
    for idx, item in enumerate(motor_list):
        dest_file = '{0}/docs/motors/{1}.markdown'.format(args.website, item['url_name'])
        with open(dest_file, 'w') as out_file:
            print('---', file=out_file)
            print('autogen:', 'This file was automatically generated by ' +
                    'motor-defs-to-markdown.py', file=out_file)
            ...
            print('title:', get_motor_page_title(item), file=out_file)
